...
| `ui/AdminUserFormPanel.jsx` | Правая панель: селекты (роль, подразделение 30/99), ФИО, логин, пароль, телефон, почта, даты (`type="date"` + кнопки сброса на сегодня), примечание, чекбоксы «Подключен» / «Первое подключение»; проп `isNewUserDraft`; внизу кнопка «Сохранить» (заглушка). | готово |
...
- **`ui/AdminUserFormPanel.jsx`** — заголовок «Новый пользователь» в режиме черновика; кнопка «Сохранить» внизу (TODO: API); дефолты нового пользователя по-прежнему из `applyDefaultsForAdd`.
