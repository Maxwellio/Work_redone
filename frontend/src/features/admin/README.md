...
- **Слева** — поиск, фильтры и таблица пользователей; справа от чекбоксов фильтра ролей — кнопка **«Добавить пользователя»** (открывает правую панель в режиме нового пользователя, без выбранной строки).
- **Справа** — форма [`AdminUserFormPanel`](ui/AdminUserFormPanel.jsx) при **выборе строки** в таблице или при нажатии **«Добавить пользователя»**. Внизу формы — кнопка **«Сохранить»** (обработчик и API сохранения — позже).
...
| `ui/AdminUserFormPanel.jsx` | Правая панель: селекты (роль, подразделение 30/99), ФИО, логин, пароль, телефон, даты (`type="date"` + кнопки сброса на сегодня), примечание, чекбоксы «Подключен» / «Первое подключение»; проп `isNewUserDraft`; внизу кнопка «Сохранить» (заглушка). | готово |
| `ui/AdminWorkspaceMock.jsx` | Левая зона: таблица, поиск, фильтр подразделения, чекбоксы ролей, кнопка «Добавить пользователя»; правая форма при выбранной строке или режиме добавления. | готово |
...
- **`ui/AdminWorkspaceMock.jsx`** — состояние `isAddUserOpen`, кнопка «Добавить пользователя» в тулбаре (справа от чекбоксов, на широком экране с отступом `ml: auto`), панель формы при `selected != null || isAddUserOpen`; сброс режима добавления при выборе строки; в форму передаётся `isNewUserDraft={isAddUserOpen && selected == null}`.
- **`ui/AdminUserFormPanel.jsx`** — заголовок «Новый пользователь» в режиме черновика; кнопка «Сохранить» внизу (TODO: API); дефолты нового пользователя по-прежнему из `applyDefaultsForAdd`; поле почты убрано.
